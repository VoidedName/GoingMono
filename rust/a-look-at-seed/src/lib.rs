...
fn update(msg: Msg, model: &mut Model, orders: &mut impl Orders<Msg>) {
    match msg {
        ...
        Msg::SaveSelectedTodo => {
            if let Some(selected_todo) = &mut model.selected_todo.take() {
    ...
    LocalStorage::set(STORAGE_KEY, &model.todos).expect("save todos to LocalStorage");
}
...
fn view_toggle_all(todos: &BTreeMap<Ulid, Todo>) -> Vec<Node<Msg>> {
    ...
    vec![
        input![
            ...
            ev(Ev::Change, |_| Msg::ToggleAllChecked),
        ],
