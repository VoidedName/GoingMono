    ...
    orders
        ...
        .perform_cmd(async {
            Msg::AuthConfigFetched(
                match Request::get("/auth_config.json")
                    .send()
                    .await
                {
                    Ok(response) => response.json::<AuthConfig>().await,
                    Err(e) => Err(e)
                }
            )
        })
...
enum Msg {
    ...
    ToggleMenu,
    HideMenu,
    AuthConfigFetched(Result<AuthConfig, gloo_net::Error>),
    ...
    match msg {
        ...
        Msg::AuthConfigFetched(Ok(auth_config)) => {
            ...
            orders.perform_cmd(async {
                Msg::AuthInitialized(init_auth(domain, client_id).await)
            });
        }
        Msg::AuthConfigFetched(Err(fetch_error)) => {
            error!("Auth config fetch failed!", fetch_error.to_string())
        }
        ...
        Msg::AuthInitialized(Err(err)) => {
            error!("Auth initialization failed!", err)
        }
        Msg::SignUp => {
            orders.perform_cmd(async {
                Msg::RedirectingToSignUp(redirect_to_sign_up().await)
            });
        }
        Msg::LogIn => {
            orders.perform_cmd(async {
                Msg::RedirectingToLogIn(redirect_to_log_in().await)
            });
        }
        Msg::LogOut => {
            if let Err(err) = logout() {
                error!("Can not log out!", err);
            } else {
                model.ctx.user = None;
            }
        }
